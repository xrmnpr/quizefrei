...
async function gradeResponse(question, r) {
  ...
  if (!choiceId) {
    return { choiceId: null, isCorrect: false };
  }
  const choice = await db.queryOne(
    'SELECT * FROM choices WHERE id = $1 AND question_id = $2',
...
router.post('/:id/submit', async (req, res) => {
  try {
    ...
    // Fetch all questions and choices up front to avoid N+1 queries
    ...
    const score = autoGradableCount > 0 ? Math.round((correct / session.total_questions) * 100) : null;
    await db.query(
      'UPDATE quiz_sessions SET completed_at = NOW(), score = $1 WHERE id = $2',
      [score, session.id]
    );

    // Build review with correct answers
    const review = await Promise.all(allQuestions.map(async q => {
      const correctChoices = await db.query(
        'SELECT id, choice_text FROM choices WHERE question_id = $1 AND is_correct = TRUE',
        [q.id]
      );
    ...
    console.error('POST /sessions/:id/submit error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});
...
router.get('/class/:classId/results', async (req, res) => {
  ...
  try {
    ...
    console.error('GET /sessions/class/:classId/results error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});
